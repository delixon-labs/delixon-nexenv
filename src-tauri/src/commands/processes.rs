...
#[command]
pub async fn kill_process(pid: u32, project_id: String) -> Result<(), String> {
    let projects = store::get().list_projects().map_err(|e| e.to_string())?;
    let project = projects
        .iter()
        .find(|p| p.id == project_id)
    ...
    let project_processes =
        processes::list_processes_on_ports(&project.path).map_err(|e| e.to_string())?;

    if !project_processes.iter().any(|p| p.pid == pid) {
        return Err(format!("PID {} no pertenece al proyecto {}", pid, project_id));
    }
